...
/**
 Абстрактный контракт дебота. "Дебот инициализация списка"
Содержит в себе методы, связанные с работой со списком покупок в целом. Прежде всего это проверка того, 
создан ли уже список покупок и все что с этим связано.
...
abstract contract DebotShop is Debot, Upgradable, IShop {
    bytes m_icon;
    ...
    function deleteTask(uint32 index) public override{
        index = index;
        if (m_stat.completeCount + m_stat.incompleteCount > 0) {
